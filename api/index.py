...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        # Очищаем путь от параметров, если они есть
        path_requested = self.path.split('?')[0]

        try:
            # 1. Если запрашивают конкретно файл sub.txt
            ...
            # 2. Во всех остальных случаях отдаем красивую страницу (Главная)
            sub_res = requests.get(GITHUB_BASE + "sub.txt", timeout=5)
